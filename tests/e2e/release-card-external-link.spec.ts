...
test("release card external link opens new tab with rel noopener", async ({
  page,
  context,
}) => {
  await login(page);
  ...
  const card = page
    .locator("div")
    .filter({ has: page.getByText("test/test") })
    .first();
  const link = card.getByRole("link", { name: "Open release", exact: true });
