...
test("disabling 'Mark as seen' hides both action buttons", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: "Mark as seen" }).first(),
  ...
  await page.goto("/en");
  await waitForRepoLink(page);
  await expect(page.getByRole("button", { name: "Mark as seen" })).toHaveCount(
    0,
  );
  await expect(page.getByRole("button", { name: "Mark as new" })).toHaveCount(
    0,
  );
...
test("disabling 'Mark as new' hides only that button when not new", async ({
  ...
  await expect(
    page.getByRole("button", { name: "Mark as new" }).first(),
  ).toBeVisible({ timeout: 10_000 });
