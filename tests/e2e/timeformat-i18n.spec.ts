...
test("time format follows locale conventions in every published locale", async ({
  ...
  const uk12 = await setFormatAndRead(page, "uk", "12");
  expect(uk12).toMatch(/дп|пп/iu);

  const uk24 = await setFormatAndRead(page, "uk", "24");
  expect(uk24).not.toMatch(/дп|пп/iu);
  expect(uk24).toMatch(/\d{1,2}:\d{2}/);
  expect(uk24).not.toBe(uk12);

  await ensureAppLocale(page, "nl");

  const nl12 = await setFormatAndRead(page, "nl", "12");
  expect(nl12).toMatch(/[ap]\.?\s*m\.?/iu);

  const nl24 = await setFormatAndRead(page, "nl", "24");
  expect(nl24).not.toMatch(/[ap]\.?\s*m\.?/iu);
  expect(nl24).toMatch(/\d{1,2}:\d{2}/);
  expect(nl24).not.toBe(nl12);

  await ensureAppLocale(page, "ru");

  const ru12 = await setFormatAndRead(page, "ru", "12");
  expect(ru12).toMatch(/AM|PM/iu);

  const ru24 = await setFormatAndRead(page, "ru", "24");
