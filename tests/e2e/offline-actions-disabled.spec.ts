...
test.describe("Offline actions disabled", () => {
  ...
  test("Home: Refresh and Export disabled offline; Header logout disabled", async ({
    page,
  }) => {
    await page.goto("/en");
  ...
  test("Repo form: add/import disabled offline", async ({ page }) => {
    ...
    await page
      ...
      .fill("https://github.com/test/test");
  ...
  test("Test page: actions disabled offline", async ({ page }) => {
    ...
    const sendEmail = page.getByRole("button", {
      name: "Send Direct Test Email",
    });
    ...
    const sendApprise = page.getByRole("button", {
