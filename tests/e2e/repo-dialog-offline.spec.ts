...
const OFFLINE_NOTICE_EN =
  "Offline – changes are read-only and auto-save is paused.";
const OFFLINE_NOTICE_DE =
  "Offline – Änderungen sind schreibgeschützt, automatisches Speichern ist pausiert.";
...
test.describe("Repo dialog offline read-only + autosave pause", () => {
  ...
  test("controls disabled offline, notice visible, reset disabled; re-enable online and autosave works", async ({
    page,
  }) => {
    ...
    const notice = dialog
      .getByText(OFFLINE_NOTICE_EN)
      .or(dialog.getByText(OFFLINE_NOTICE_DE));
    ...
    await expect(
      dialog
        ...
        .or(dialog.getByLabel("Ausschließen-Muster (Exclude)")),
    ).toBeDisabled();
    ...
    await expect(
      dialog.getByText(/Offline – saving paused|Offline – Speichern pausiert/),
    ).toBeVisible();
  ...
  test("scroll container allows reaching bottom content", async ({ page }) => {
    ...
    const appriseLabel = dialog
      .getByRole("heading", { name: "Apprise Settings" })
      .or(dialog.getByRole("heading", { name: "Apprise-Einstellungen" }));
    // In case it's already visible, still perform a scroll to bottom
    await dialog.evaluate((node) => {
      const scrollers = Array.from(
        node.querySelectorAll("div"),
      ) as HTMLElement[];
      const target = scrollers.find(
        (el) =>
