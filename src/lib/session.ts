...
// Type declaration for global flags
declare global {
  var _authSecretChecked: boolean;
  var _httpWarningIssued: boolean;
}

// Initialize flags to ensure they always have a boolean value
global._authSecretChecked ??= false;
global._httpWarningIssued ??= false;
...
if (!global._authSecretChecked) {
  if (!process.env.AUTH_SECRET || process.env.AUTH_SECRET.length < 32) {
    logger
      .withScope("Session")
      .error(
        "CRITICAL: Missing or insecure AUTH_SECRET. Must be at least 32 characters long. Please check your .env file.",
      );
  }
  // Set the flag to true after the first check to prevent re-running.
  global._authSecretChecked = true;
}

// 2. Warn if running in production over HTTP.
// This block also runs only once at startup.
if (!global._httpWarningIssued) {
  ...
  global._httpWarningIssued = true;
}
